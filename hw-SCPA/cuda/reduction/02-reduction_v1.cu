...
int main(int argc, char** argv) {
  ...
  reduce_alloc_wrk(); //FANFA: l'allocazione avviene qui in modo tale che non venga conteggiata nelle performance (trattasi di overhead, non di lavoro utile).
  timer->start();
  
  double gpu_sum = gpu_reduce(N,d_x);

  timer->stop();
  // FANFA: compute the effective bandwidth in GB/s.
  // Note: the timer is in milliseconds! 
  bdwdth = (N*sizeof(double))/(timer->getTime()*1000000);
